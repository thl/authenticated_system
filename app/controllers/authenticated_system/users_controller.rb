class AuthenticatedSystem::UsersController < AclController
  ...
  def edit
    @user = @person.user
    @roles = AuthenticatedSystem::Role.order('title')
  end
  ...
  def update_roles(user_roles)
    ...
    roles_to_create.each { |role_id| AuthenticatedSystem::Role.find(role_id).users << @user }

    roles_to_delete = already_assigned_roles - new_roles
    roles_to_delete.each {|role_id| AuthenticatedSystem::Role.find(role_id).users.delete(@user)}
  end
